...
class KuotaDospem_model extends CI_Model
{
    protected $table = 'kuota_dospem';

    public function get()
    {
        $this->db->select("*");
        $kuotadospem = $this->db->get($this->table)->result_array();

        $hasil['error'] = false;
        $hasil['message'] = ($kuotadospem) ? "data berhasil ditemukan" : "data tidak tersedia";
        $hasil['data'] = $kuotadospem;
    ...
    {
        $kuotadospem = $this->db->get_where($this->table, array('id' => $this->input->post('id')))->result();
